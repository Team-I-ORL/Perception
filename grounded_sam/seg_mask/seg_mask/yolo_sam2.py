...
class SegMaskService(Node):
    def __init__(self):
        ...
        # Load the models
        self.yolo_model = YOLO('/home/siddharth/fall_ws/src/Perception/grounded_sam/seg_mask/checkpoints/yolo_best.pt')
    ...
    def seg_mask_service(self, request, response):
        try:
            ...
            masks = []
            for mask in masks:
                mask_data = mask.data.cpu().numpy().astype(np.int8)
                mask_area = np.sum(mask_data)
                areas.append(mask_area)
                masks.append(mask_data*255)
            ...
            response.segmask = self.bridge.cv2_to_imgmsg(gray_segmented_image, "mono8")
            return response

        except Exception as e:
            self.get_logger().error(f"Failed to process the request: {e}")
            response.segmask = self.bridge.cv2_to_imgmsg(cv2.cvtColor(np.zeros_like(color_image), cv2.COLOR_BGR2GRAY), "mono8")
            return None
